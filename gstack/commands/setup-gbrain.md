...
$HOME/.gstack/runtime/bin/gstack-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$HOME/.gstack/runtime/bin/gstack-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
$HOME/.gstack/runtime/bin/gstack-gbrain-detect
```
...
**If A (Retry)**: re-run `$HOME/.gstack/runtime/bin/gstack-gbrain-detect`
...
```bash
$HOME/.gstack/runtime/bin/gstack-gbrain-install
```
...
```bash
. $HOME/.gstack/runtime/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | $HOME/.gstack/runtime/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$($HOME/.gstack/runtime/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$($HOME/.gstack/runtime/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
$HOME/.gstack/runtime/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$($HOME/.gstack/runtime/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  $HOME/.gstack/runtime/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
$HOME/.gstack/runtime/bin/gstack-gbrain-install || exit $?
...
```bash
current_tier=$($HOME/.gstack/runtime/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  $HOME/.gstack/runtime/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
$HOME/.gstack/runtime/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
$HOME/.gstack/runtime/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
$HOME/.gstack/runtime/bin/gstack-gbrain-source-wireup --strict \
...
```bash
$HOME/.gstack/runtime/bin/gstack-memory-ingest --probe
```
...
After answer:
```bash
$HOME/.gstack/runtime/bin/gstack-config set transcript_ingest_mode <choice>
$HOME/.gstack/runtime/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
```bash
_HASH=$($HOME/.gstack/runtime/bin/gstack-config endpoint-hash 2>/dev/null)
_POLICY=$($HOME/.gstack/runtime/bin/gstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
$HOME/.gstack/runtime/bin/gstack-config set brain_trust_policy@$_HASH personal
...
```bash
$HOME/.gstack/runtime/bin/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$($HOME/.gstack/runtime/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  $HOME/.gstack/runtime/bin/gstack-config set artifacts_sync_mode full
...
```bash
$HOME/.gstack/runtime/bin/gstack-gbrain-detect 2>/dev/null || true
$HOME/.gstack/runtime/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
$HOME/.gstack/runtime/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
