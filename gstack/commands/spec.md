...
$HOME/.gstack/runtime/bin/gstack-timeline-log '{"skill":"spec","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$HOME/.gstack/runtime/bin/gstack-question-log '{"skill":"spec","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
bun $HOME/.gstack/runtime/lib/redact-audit-log.ts \
...
REDACT_VIS=$($HOME/.gstack/runtime/bin/gstack-config get redact_repo_visibility 2>/dev/null)
...
REDACT_BODY_EOF
REDACT_JSON=$($HOME/.gstack/runtime/bin/gstack-redact --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json)
REDACT_CODE=$?
```
...
reuse it; write the exact bytes to `$REDACT_FILE`; `$HOME/.gstack/runtime/bin/gstack-redact --from-file "$REDACT_FILE"
...
$HOME/.gstack/runtime/bin/gstack-decision-log '{"decision":"Spec filed #ISSUE_NUMBER: TITLE","rationale":"APPROACH","scope":"issue","issue":"ISSUE_NUMBER","source":"skill","confidence":7}' 2>/dev/null || true
```
...
eval "$($HOME/.gstack/runtime/bin/gstack-slug)"
