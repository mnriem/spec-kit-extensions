...
[ -z "$_AUTO" ] && _AUTO=$($HOME/.gstack/runtime/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
$HOME/.gstack/runtime/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
$HOME/.gstack/runtime/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `$HOME/.gstack/runtime/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/.gstack/runtime/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/runtime"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d "$HOME/.gstack/runtime/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR="$HOME/.gstack/runtime"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d "$HOME/.gstack/runtime" ]; then
...
_TEAM_MODE=$($HOME/.gstack/runtime/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached $HOME/.gstack/runtime/ 2>/dev/null || true
if ! grep -qF '$HOME/.gstack/runtime/' .gitignore 2>/dev/null; then
  echo '$HOME/.gstack/runtime/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `$HOME/.gstack/runtime/` when you're ready."
...
```bash
$HOME/.gstack/runtime/bin/gstack-update-check --force 2>/dev/null || \
$HOME/.gstack/runtime/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `$HOME/.gstack/runtime/` when you're ready."
