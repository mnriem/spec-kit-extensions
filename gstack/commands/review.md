...
$HOME/.gstack/runtime/bin/gstack-timeline-log '{"skill":"review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$HOME/.gstack/runtime/bin/gstack-question-log '{"skill":"review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
$HOME/.gstack/runtime/bin/gstack-learnings-log '{
  "type": "pitfall",
...
```bash
_CROSS_PROJ=$($HOME/.gstack/runtime/bin/gstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  $HOME/.gstack/runtime/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  $HOME/.gstack/runtime/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `$HOME/.gstack/runtime/bin/gstack-config set cross_project_learnings true`
If B: run `$HOME/.gstack/runtime/bin/gstack-config set cross_project_learnings false`
...
```bash
source <($HOME/.gstack/runtime/bin/gstack-diff-scope <base> 2>/dev/null) || true
...
```bash
$HOME/.gstack/runtime/bin/gstack-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `$HOME/.gstack/runtime/review/specialists/testing.md`
2. **Maintainability** — read `$HOME/.gstack/runtime/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `$HOME/.gstack/runtime/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `$HOME/.gstack/runtime/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `$HOME/.gstack/runtime/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `$HOME/.gstack/runtime/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `$HOME/.gstack/runtime/review/design-checklist.md`
...
```bash
$HOME/.gstack/runtime/bin/gstack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `$HOME/.gstack/runtime/review/specialists/red-team.md`
...
_TEL=$($HOME/.gstack/runtime/bin/gstack-config get telemetry 2>/dev/null || echo off)
_CODEX_CFG=$($HOME/.gstack/runtime/bin/gstack-config get codex_reviews 2>/dev/null || echo enabled)
source $HOME/.gstack/runtime/bin/gstack-codex-probe 2>/dev/null || true
...
```bash
$HOME/.gstack/runtime/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$HOME/.gstack/runtime/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"COMMIT"}'
```
...
```bash
$HOME/.gstack/runtime/bin/gstack-learnings-log '{"skill":"review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
