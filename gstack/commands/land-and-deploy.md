...
$HOME/.gstack/runtime/bin/gstack-timeline-log '{"skill":"land-and-deploy","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$HOME/.gstack/runtime/bin/gstack-question-log '{"skill":"land-and-deploy","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
$HOME/.gstack/runtime/bin/gstack-review-read 2>/dev/null
```
...
```bash
cat $HOME/.gstack/runtime/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
```bash
eval $($HOME/.gstack/runtime/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
