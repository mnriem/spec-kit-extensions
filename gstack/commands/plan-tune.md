...
$HOME/.gstack/runtime/bin/gstack-timeline-log '{"skill":"plan-tune","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$HOME/.gstack/runtime/bin/gstack-question-log '{"skill":"plan-tune","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
10. **"Turn it off" / "disable"** → `$HOME/.gstack/runtime/bin/gstack-config set question_tuning false`
11. **"Turn it on" / "enable"** → `$HOME/.gstack/runtime/bin/gstack-config set question_tuning true && touch ~/.gstack/.question-tuning-prompted`
...
1. Detect contributor state (for prompt framing only, not for auto-action):
   ...
   _CONTRIB=$($HOME/.gstack/runtime/bin/gstack-config get gstack_contributor 2>/dev/null || echo "false")
...
4. If A or B: enable:
   ```bash
   $HOME/.gstack/runtime/bin/gstack-config set question_tuning true
   ```
...
1. Ask FIVE one-per-dimension declaration questions via individual
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   $HOME/.gstack/runtime/bin/gstack-developer-profile --read >/dev/null
   # Update declared dimensions atomically
   eval "$($HOME/.gstack/runtime/bin/gstack-paths)"
...
```bash
$HOME/.gstack/runtime/bin/gstack-developer-profile --profile
```
...
4. Write:
   ```bash
   $HOME/.gstack/runtime/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
$HOME/.gstack/runtime/bin/gstack-developer-profile --gap
```
...
```bash
$HOME/.gstack/runtime/bin/gstack-question-preference --stats
eval "$($HOME/.gstack/runtime/bin/gstack-slug 2>/dev/null)"
eval "$($HOME/.gstack/runtime/bin/gstack-paths)"
_LOG="$GSTACK_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ -f "$_LOG" ]; then
...
$HOME/.gstack/runtime/bin/gstack-developer-profile --profile | bun -e "
...
"
echo '---DISTILL---'
$HOME/.gstack/runtime/bin/gstack-distill-free-text --status
```
...
1. Show the proposals:
   ```bash
   $HOME/.gstack/runtime/bin/gstack-distill-apply --list
   ```
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   ...
   $HOME/.gstack/runtime/bin/gstack-distill-apply --proposal N --gbrain-published true|false
   ```

   For `preference`:
   ```bash
   $HOME/.gstack/runtime/bin/gstack-distill-apply --proposal N
   ```
...
1. Run distill:
   ```bash
   $HOME/.gstack/runtime/bin/gstack-distill-free-text
   ```
...
```bash
$HOME/.gstack/runtime/bin/gstack-distill-free-text --background
```
